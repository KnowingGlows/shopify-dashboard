...
import { Wallet, Package, Receipt, Plus, Trash2, Download, ChevronDown, X } from 'lucide-react';
import { PageTransition } from '@/components/motion';
import { formatINR, formatUSD } from '@/lib/currency-converter';
import { compute3PL, productBeroasOf, type FulfilmentMode } from '@/lib/3pl';
...
function blankProduct(): PProduct {
  return { id: uid(), name: '', mode: '3pl', sellingPrice: '0', cogs: '0', deliveryRate: '0', ownPackingCost: '0', dailySpend: '0', roas: '0', dailyRoas: [] };
}
...
interface Flow {
  ...
  ads: number; cogs: number; fees: number; gst: number;
  net: number;
}
function dailyFlow(p: PProduct, spend: number, roas: number): Flow {
  ...
  if (p.mode === 'own') {
    ...
    const outputGstPerDel = sp * gstRate / (1 + gstRate);
    const inputGstPerOrder = (cogs + packing) * gstRate;
    const gst = delivered * (outputGstPerDel - inputGstPerOrder) + rto * (-inputGstPerOrder);
    const revColl = delivered * sp;
    const cogsNet = delivered * cogs;            // RTO units recovered to stock
    const fees = orders * packing + orders * payment;
    const net = revColl - cogsNet - fees - gst - spend;
    return { orders, delivered, revBooked: booked, revColl, ads: spend, cogs: cogsNet, fees, gst, net };
  }
  ...
  const r = compute3PL({
    ...
    spGstInclusive: true, chargeOutputGst: true,
  });
  return {
    orders: r.shipped, delivered: r.delivered,
    revBooked: r.shopifyRevenue, revColl: r.moneyIn,
    ads: r.outAds, cogs: r.outCOGS - r.stockRecovered, fees: r.total3PL, gst: r.outGst,
    net: r.netProfit,
  };
}

const ZERO: Flow = { orders: 0, delivered: 0, revBooked: 0, revColl: 0, ads: 0, cogs: 0, fees: 0, gst: 0, net: 0 };
function addFlow(a: Flow, b: Flow): Flow {
  return {
    ...
    ads: a.ads + b.ads, cogs: a.cogs + b.cogs, fees: a.fees + b.fees, gst: a.gst + b.gst,
    net: a.net + b.net,
  };
}

/** Sum a product's flows across the horizon, using each day's logged ROAS (or expected). */
function projectProduct(p: PProduct, horizon: number): Flow {
  ...
  for (let day = 0; day < horizon; day++) {
    ...
    acc = addFlow(acc, dailyFlow(p, spend, roas));
  }
  return acc;
}
...
export default function PlannerPage() {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      setSaving(true);
      fetch('/api/planner', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ products, expenses, horizonDays: horizon }) })
        .catch(() => {}).finally(() => setSaving(false));
    }, 800);
    return () => { if (saveTimer.current) clearTimeout(saveTimer.current); };
  }, [products, expenses, horizon, loaded]);
  ...
  // ── Derived ───────────────────────────────────────────────────────────
  const horizonFlows = useMemo(() => products.map((p) => ({ p, day: dailyFlow(p, n(p.dailySpend), n(p.roas)), horizon: projectProduct(p, horizon) })), [products, horizon]);
  const portfolio = useMemo(() => horizonFlows.reduce((acc, f) => addFlow(acc, f.horizon), ZERO), [horizonFlows]);
  const expensesOverHorizon = useMemo(() => expenses.reduce((s, e) => s + (e.basis === 'day' ? n(e.amount) * horizon : n(e.amount) * (horizon / 30)), 0), [expenses, horizon]);
  const trueNet = portfolio.net - expensesOverHorizon;
  const blendedRoas = portfolio.ads > 0 ? portfolio.revBooked / portfolio.ads : 0;

  // Day labels for the daily-ROAS log.
    ...
    <PageTransition className="mx-auto max-w-7xl p-5 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">Per-product money in &amp; out — fixed daily spend, logged ROAS, {horizon}-day projection</p>
        </div>
      ...
      {/* ── Bird's-eye — portfolio over the horizon ─────────────────────── */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        className="rounded-xl border border-border bg-card overflow-hidden card-hover-glow">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
          <Wallet className="h-4 w-4 text-primary" />
          <div>
            <h2 className="text-sm font-semibold text-foreground">Bird&apos;s-eye — next {horizon} days</h2>
            <p className="text-[10px] text-muted-foreground">{products.length} product{products.length === 1 ? '' : 's'} · where the money comes from &amp; goes</p>
          </div>
        </div>
        <div className="relative z-10 p-4 grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="space-y-1.5">
            ...
            <ResultRow label="Fulfilment &amp; fees" value={fmt(portfolio.fees)} />
            <ResultRow label="GST (net)" value={fmt(portfolio.gst)} />
            <ResultRow label="Operating expenses" value={fmt(expensesOverHorizon)} />
          </div>
          {/* Bottom line + hero */}
          <div className="space-y-1.5">
            <ResultRow label="Product net (before expenses)" value={fmt(portfolio.net)} highlight="primary" />
            <ResultRow label="− Operating expenses" value={`− ${fmt(expensesOverHorizon)}`} />
            <ResultRow label="True net profit" value={fmt(trueNet)} highlight={trueNet >= 0 ? 'success' : undefined} />
            <ResultRow label="Orders (booked)" value={Math.round(portfolio.orders).toLocaleString('en-IN')} />
            <ResultRow label="Delivered orders" value={Math.round(portfolio.delivered).toLocaleString('en-IN')} />
            ...
            <div className="stat-shimmer glow-pulse rounded-xl border border-emerald-500/20 bg-emerald-500/[0.04] p-6 text-center mt-2"
              ...
              <p className="relative z-10 text-[10px] font-medium uppercase tracking-wider text-muted-foreground mb-1">True net · {horizon} days</p>
              <p className={`relative z-10 text-4xl font-bold font-mono ${trueNet >= 0 ? 'gradient-text-emerald' : 'text-rose-400'}`}>{fmt(trueNet)}</p>
              <p className="relative z-10 mt-1.5 text-[11px] text-muted-foreground">{fmt(trueNet / Math.max(1, horizon))}/day · {portfolio.revColl > 0 ? ((trueNet / portfolio.revColl) * 100).toFixed(1) : '0'}% of collected</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      {products.length === 0 ? (
        ...
        <div className="space-y-4">
          {horizonFlows.map(({ p, day, horizon: proj }, idx) => {
            const beroas = productBeroasOf({ sellingPrice: n(p.sellingPrice), cogs: n(p.cogs), deliveryRate: n(p.deliveryRate), fulfilmentMode: p.mode, ownPackingCost: n(p.ownPackingCost) });
            ...
            const moneyOut = day.ads + day.cogs + day.fees + Math.max(0, day.gst);
              ...
              <motion.div key={p.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + idx * 0.03 }}
                ...
                <div className="relative z-10 p-4 grid grid-cols-1 lg:grid-cols-2 gap-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <FlowRow label="Ad spend" amount={day.ads} total={moneyOut} fmt={fmt} />
                    <FlowRow label="Product cost (COGS, net)" amount={day.cogs} total={moneyOut} fmt={fmt} />
                    <FlowRow label={p.mode === 'own' ? 'Packing + payment fee' : '3PL fulfilment & fees'} amount={day.fees} total={moneyOut} fmt={fmt} />
                    <FlowRow label="GST (net)" amount={day.gst} total={moneyOut} fmt={fmt} />
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
          ...
          <div>
            <h2 className="text-sm font-semibold text-foreground">Operating expenses</h2>
            <p className="text-[10px] text-muted-foreground">Everything outside product economics — folded into the true net</p>
          </div>
        ...
        <div className="relative z-10 p-4 space-y-2">
          {expenses.length === 0 && <p className="py-3 text-center text-[12px] text-muted-foreground/60">No expenses yet — salaries, tools, rent, etc.</p>}
          {expenses.map((e) => (
            <div key={e.id} className="flex items-center gap-2">
              <input value={e.label} onChange={(ev) => setExp(e.id, { label: ev.target.value })} placeholder="e.g. Salaries, SaaS, Rent" className="flex-1 rounded-lg border border-border bg-background/40 px-3 py-2 text-[12px] text-foreground outline-none placeholder:text-muted-foreground/40 focus:border-primary/40" />
              ...
              <span className="w-24 text-right font-mono text-[11px] tabular-nums text-muted-foreground">{fmt(e.basis === 'day' ? n(e.amount) * horizon : n(e.amount) * (horizon / 30))}</span>
